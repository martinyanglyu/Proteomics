...
knit: (
  function(inputFile, encoding) {
    ...
    pSubTitle <- "3_NC200_subtype_Analysis.pdf"
...
# NC data (232 samples)
**Proteomics data:  Rawdata/NC_230/data.MBR.int.rm_NA.txt"**
**clinial_data**


```{r}
dir(Rawdata_path)
MBR_data=read.table(file.path(Rawdata_path,"NC_230","data.MBR.int.rm_NA.txt"),fill=T,header=T,sep="\t" )
MBR_data2=MBR_data%>%dplyr::select(-Genes.ID)%>%dplyr::select(-Gene.Symbol)
rownames(MBR_data2)=MBR_data$Gene.Symbol

Normal=MBR_data2[,grepl("TA",colnames(MBR_data2))]
Tumor=MBR_data2[,grepl("T$",colnames(MBR_data2))]

Data_match=data.frame(Normal=gsub("_TA","",colnames(Normal)) ,Tumor=gsub("_T","",colnames(Tumor)))
Data_match$Normal==Data_match$Tumor

# Tumor/normal
Pro_log_data=log2(Tumor+1)-log2(Normal+1)
Pro_log_data_rm_na=Pro_log_data%>%na.omit()%>%as.data.frame()

load(file.path(Rawdata_path,"NC_230", "T2_T4_subtype_clin.rdata")  ) #CLINICAL DATA
```


```{r}
if( !file.exists(file.path(Rawdata_path,"NC_230","NC230_Singscore.rdata"))){
...
all_pathways2=c()
 ...
 save( KIRC_singscore_data,file=file.path(Rawdata_path,"NC_230","NC230_Singscore.rdata"  ))
} else {
   
   load (file.path(Rawdata_path,"NC_230","NC230_Singscore.rdata"  ))
 }
...
sig_pathways_umap=UMAP_output(df)%>% left_join(.,km.res_anno[,c("Sample_ID","Cluster")]) %>%mutate(Cluster=as.character(Cluster))%>%mutate(Subtype=ifelse(Cluster==1,"NC_type2","NC_type1"))
...
Norm_singscore_anno$Subtype=factor(Norm_singscore_anno$Subtype,levels=c("NC_type1","NC_type2"))
...
write.csv(NC_subtype_anno,file.path(Out_table_path,"NC200_subtypes.csv"))
```

```{r,fig.height=8,fig.width=8}
library(survival) 
T3_T4_anno=NC_200_T2_T4_subtype_clin%>%left_join(.,NC_subtype_anno,by="Sample_ID") 
...
Psur_1=survminer::ggsurvplot(fit,pval = TRUE,title="T1-T4",risk.table = T,data=T3_T4_anno,palette = cbp1[c(7,8)])
...
```{r,fig.height=8,fig.width=8}
library(survival) 
T3_T4_anno=NC_200_T2_T4_subtype_clin%>%left_join(.,NC_subtype_anno,by="Sample_ID") %>%dplyr::filter(stage2!="pT1")

setdiff(NC_200_T2_T4_subtype_clin$Sample_ID,NC_subtype_anno$Sample_ID)
T3_T4_anno$Subtype2=factor(T3_T4_anno$Subtype2,levels = c("NC_type1","NC_type2"))
fit<- survival::survfit(survival::Surv(Month, Status)~Subtype2, data =T3_T4_anno)
# Drawing survival curves
Psur_2=survminer::ggsurvplot(fit,pval = TRUE,title="T2-T4",risk.table = T,data=T3_T4_anno,palette = cbp1[c(7,8)])
...
```{r,fig.width=30,fig.height=16,include=T}
cowplot::plot_grid(P3,labels = c("A"))
```
...
cowplot::plot_grid(P2,grid::nullGrob() ,labels = c("B",""))

```


```{r,fig.width=10,fig.height=6,include=T}
#cowplot::plot_grid(PHEATMAP$gtable,P4,nrow=1,labels = c("A","B"),scale=c(1,0.8))

cowplot::plot_grid(out,grid::nullGrob() ,labels = c("C",""))
...
\newpage
```{r,fig.height=38,fig.width=60,include=T,out.width="80%" }
...
PHEATMAP=pheatmap::pheatmap(Singscore_data_t_select,cellwigermline=5, cellheight = 30,cellwidth = 10, scale = "row",
                                    treeheight_row = 5,fontsize_row = 10, 
...
```{r,fig.height=40,fig.width=90,include=T}
...
outlist=list()
Type1_ID=sig_pathways_umap[,c("Sample_ID","Subtype")]%>%dplyr::filter(Subtype=="NC_type1")%>%.$Sample_ID
Type2_ID=sig_pathways_umap[,c("Sample_ID","Subtype")]%>%dplyr::filter(Subtype=="NC_type2")%>%.$Sample_ID
...
#write.csv(wilcox_test_out_sig, file.path(Rawdata_path,"NC","Proteomics","CHKIRC_DEPs_in_subtypes.csv" ))
```
...
Valcano_plot_all_proteins(wilcox_list_out,"All DEPs in NC_type2 vs NC_type1")
```
...
```{r}
Subtype1_ID=sig_pathways_umap%>%dplyr::filter(Subtype=="NC_type1")%>%.$Sample_ID
